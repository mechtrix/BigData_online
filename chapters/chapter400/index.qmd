...
virtual_resample <- coins_sample %>% 
  infer::rep_sample_n(size = 50, replace = TRUE,reps = 1000)

# virtual_resample01 <- coins_sample %>%
...
3. Z-Score method: Calculate the Z-score ($Z = \frac{x_i - \bar{x}}{sd}$) and setz the threshold at $\pm2$ or $\pm3$
...
- The $x_i$ iare sampled from a uniform distribution over $[0, 1]$.
- The $x_i$ and $\epsilon$ are independent.
- The $y_i$ in are sampled from the normal distribution.
